 ...
 */
export class SpritePerspectiveController
  extends Component
  implements ISpriteController
{
  ...
  initialize(
    ...
    this.setSpritePosition(0, 100, 0);
    this.scale(10);
    this.setSprite('ness.left.stand');
    this.commitToBuffer();
  ...
  update(dt: number) {
    if (!this._buffer.buffersCreated) {
      ...
      //proj.setIdentity();
      //const view = mat4.lookAt(vec3.zero, new vec3([0, 50, 1]));
      //view.setIdentity();

      const point0 = this._sprite.quad.min;
      const point1 = this._sprite.quad.max;
      const transform0 = new vec4([
        point0[0],
        point0[1],
        this._sprite.depth,
        1.0,
      ]).multiplyMat4(proj);
      const transform1 = new vec4([
        point1[0],
        point1[1],
        this._sprite.depth,
        1.0,
